...
class BaseMailTemplate:
    ...
    def get_subject(self, context=None):
        return "".join(render_template(self.subject_template_name, **(context or {})).splitlines())

    def get_body(self, context=None, is_html_template=False):
        body_template = self.body_html_template_name if is_html_template else self.body_template_name
