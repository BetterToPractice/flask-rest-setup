...
from project.configs.settings import APP_DIR
...
class PluggyExtension:
    ...
    def init_app(self, app: Flask, plugin_name="plugins", hook_specs=None, apps_paths="apps") -> None:
        ...
        self.pm = core_pluggy.PluginManager(self.plugin_name)
        if hook_specs:
            self.pm.add_hookspecs(hook_specs)

        for module_name, module in get_modules(APP_DIR, apps_paths).items():
            self.pm.register(module, module_name)

        app.extensions["pluggy"] = self.pm
